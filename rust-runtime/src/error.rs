...
impl RpcError {
    ...
    /// envelope encoding — must stay in sync with the `RpcErrorCode` enums in
    /// the C# and Kotlin runtimes.
    ...
    /// `Application` errors so a version-skewed peer's error still surfaces
