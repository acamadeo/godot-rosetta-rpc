...
/**
 * Stable, wire-level discriminant for an [RpcException]. Must stay in sync with `RpcError` (Rust)
 * and `RpcErrorCode` (C#).
 */
enum class RpcErrorCode(val wireValue: Int) {
  ...
  companion object {
    fun fromWireValue(value: Int): RpcErrorCode = entries.find { it.wireValue == value } ?: APPLICATION
  }
}
