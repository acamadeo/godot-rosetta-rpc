...
/// <see cref="RpcClient"/>/<see cref="ServiceRegistry"/> dispatch boundary, so
/// a call failure can travel back to the caller as data instead of relying on
/// an exception surviving Godot's <c>Variant</c> call boundary between
/// language runtimes.
///
/// Deliberately hand-rolled rather than a protobuf message: this framing is
/// purely internal to this library, never part of a project's <c>.proto</c>
/// schema. The Rust and Kotlin runtimes implement the same framing — keep
/// them in sync if this changes.
///
/// <code>
